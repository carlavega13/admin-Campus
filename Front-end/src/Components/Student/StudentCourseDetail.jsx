...
const StudentCourseDetail=()=>{
    ...
    if(!course?.grades){
dispatch(getStudentGrades({domain:user.domain,token:user.token,courseid:course.id,userid:user.id})).then(res=>setPromise(true))
return(
    <div>
        LOADING...
    </div>
    ...
    <h1>Calificaciones del curso: {`${course?.fullname}`}</h1>
    {
        course?.grades?.map(act=>{
            return(
                <div key={act.id} style={{display:"flex",justifyContent:"space-between"}}>
