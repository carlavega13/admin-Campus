...
const EditProfile = () => {
    ...
    if (
      ...
      if (confirm("Estas seguro que quieres editar tu información?")) {
        ...
        if (profile.firstName) {
          info.firstname = `${profile.firstName[0].toUpperCase()}${profile.firstName.slice(
            1
          )}`;
        }
        if (profile.lastName) {
          info.lastname = `${profile.lastName[0].toUpperCase()}${profile.lastName.slice(
            1
          )}`;
        }
        if (profile.phone1) {
  ...
  return (
    <div className={s.box}>
      ...
      <button onClick={handleSend} className={s.btnSubmit}>
